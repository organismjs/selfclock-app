...
function getActiveWindowData() {
  activeWindow().then((response: any) => {
    win?.webContents.send("active-window-data", response);
  });
  activeWindow.getOpenWindows().then((response: any) => {
    win?.webContents.send("active-window-data-all", response);
  });
  si.networkStats()
    .then((data: any) => win?.webContents.send("network", data))
    .catch((error: any) => console.error(error));
  si.osInfo()
    .then((data: any) => win?.webContents.send("os", data))
    .catch((error: any) => console.error(error));
}
...
function createWindow() {
  ...
  win.webContents.on("did-finish-load", () => {
    ...
    setInterval(getActiveWindowData, 2000);
  });
  ...
  tray.on("click", (_, bounds) => {
    ...
    const winBound = win?.getBounds() || { width: 0, height: 0 };

    if (win?.isVisible()) {
      win?.hide();
    } else {
      const yPosition = process.platform === "darwin" ? y : y - winBound.height;
      win?.setBounds({
        x: x - winBound?.width / 2,
        y: yPosition,
